...
export async function whatCardIsInFrontOfTheCamera(
  ip: string = "192.168.178.61"
) {
  ...
  const fullPath = process.cwd() + "/server/data/temp.jpg";
  console.log("Image gotten from camera and saved to fullPath " + fullPath);

  const name = await ocrImage(fullPath);
    ...
    messages: [
      {
        role: "user",
        content:
          // "This is a magic the gathering card. What is the title of the card on the image? You only see a portion.",
          "What magic the gathering card is this? Return just the name, without intro or header info. If there is no card, return nothing. Only return the name, nothing else. ONLY THE NAME",
        // "Act as an Magic the Gathering OCR assistant. Analyze the provided image and:" +
        // "Recognize all visible text in the image as accurately as possible." +
        // "Provide only the transcription without any additional comments" +
        // "Only return the name that you find in the top of the card, nothing else. ONLY THE NAME",
        images: [fileBase64],
      },
    ],
  });
