...
// Declaration of C function bridge implemented in shell_cli.cpp
uint32_t millis();


/**
 * Linked list of structures containing the available commands and their
 * associated function entry point, help, etc. other data required by the
 * commands may be added to this structure
 */
shell_command_entry		*p_command_list;
...
void shell_register_command( shell_command_entry *psCommandStruct )
{
		// Add the new command to the linked list
	psCommandStruct->psNext = p_command_list;
	p_command_list = psCommandStruct;
}
...
void shell_print_commands()
{
	...
	for( p_command = p_command_list; p_command; p_command = p_command->psNext )
	{
		if( p_command->shell_use_string )
		{
			shell_print( " " );
			shell_println( p_command->shell_use_string );
		}
	}
}
...
void shell_print_command_help( const char *command )
{
	...
	for( p_command = p_command_list; p_command; p_command = p_command->psNext )
	{
		if( p_command->shell_command_string && p_command->shell_help_string )
		{
			if( strcasecmp( command, p_command->shell_command_string ) == 0)
			{
					// Print the appropriate help string
				shell_println( p_command->shell_help_string );

				return;		// no reason to keep searching
			}
		}
	}
}
...
void shell_task()
{
	...
	if (shell_reader(&rxchar)) {
		...
		if (cc) {
			...
			// sequential search on command list
			for( p_command = p_command_list; p_command; p_command = p_command->psNext )
			{
				if( p_command->shell_command_string && p_command->shell_program )
				{
					if( strcasecmp( argv_list[0], p_command->shell_command_string) == 0)
					{
						// Run the appropriate function
						retval = p_command->shell_program(argc, argv_list);
						cc = false;
					}
				}
			}
