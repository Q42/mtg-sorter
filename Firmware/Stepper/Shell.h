 ...
 */
typedef struct shell_command_entry
{
	const char 					*shell_command_string;
	shell_program_t 			shell_program;
	const char 					*shell_use_string;
	const char 					*shell_help_string;
	struct shell_command_entry	*psNext;
} shell_command_entry;
...
extern "C" {
	 ...
	 *
	 * @param psCommandStruct A data structure that defines the new command
	 * and help strings.  The structure must be static, and must be able to be
	...
	/**
	 * @brief Prints the help for a specific command
	 *
	 * Prints a list of available commands to the terminal using the callback
	 * functions provided on initialization.
	 * 
